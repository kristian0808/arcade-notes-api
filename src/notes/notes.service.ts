...
export class NotesService {
  ...
  async create(createNoteDto: CreateNoteDto): Promise<CreateNoteResponseDto> {
    ...
    const contextPcName: string | undefined = undefined; // Optional context
    ...
    if (memberId) {
      ...
      try {
        ...
        if (memberDetails?.member_account) {
          ...
          this.logger.log(
            `Verified Member ID ${memberId}. Account: ${targetMemberAccount}`,
          );
        } else {
          throw new NotFoundException(
            `Member ID ${memberId} could not be verified.`,
          );
        }
      } catch (error) {
        ...
        throw new InternalServerErrorException(
          `Failed to verify member before creating note.`,
        );
      }
    }
    // APPROACH 2: Member Account is provided
      ...
      this.logger.log(
        `Note for Member Account: ${memberAccount}. Verifying...`,
      );
      try {
        const memberDetails =
          await this.icafeService.getMemberByAccount(memberAccount);

        if (memberDetails?.member_id && memberDetails?.member_account) {
          targetMemberId = memberDetails.member_id;
          targetMemberAccount = memberDetails.member_account;
          this.logger.log(
            `Verified Member Account ${memberAccount}. ID: ${targetMemberId}`,
          );
        } else {
          throw new NotFoundException(
            `Member with account '${memberAccount}' could not be verified.`,
          );
        }
      } catch (error) {
        if (error instanceof NotFoundException) {
        ...
        throw new InternalServerErrorException(
          `Failed to verify member account before creating note.`,
        );
      }
    }
    // APPROACH 3: Only PC name is provided (look up current member on that PC)
      ...
      this.logger.log(
        `Note created via PC: ${pcName}. Finding current member...`,
      );
      // contextPcName = pcName; // Record PC for context
      throw new BadRequestException(
        'Note creation requires a member context: provide memberId or memberAccount.',
      );
      ...
      // No context provided
      throw new BadRequestException(
        'Note creation requires a member context: provide memberId, memberAccount, or pcName.',
      );
    }
    ...
    if (targetMemberId === null || targetMemberAccount === null) {
      throw new InternalServerErrorException(
        'Critical error: Failed to determine target member for the note.',
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `Note created successfully for member ${targetMemberAccount} with ID: ${savedNote._id}`,
      );
      ...
      this.logger.error(
        `Failed to save note to database: ${error.message}`,
        error.stack,
      );
    ...
    // Priority 2: If memberAccount is provided, get the corresponding memberId
      ...
      try {
        ...
        if (memberDetails?.member_id) {
          ...
          this.logger.log(
            `Found memberId ${memberDetails.member_id} for account ${memberAccount}`,
          );
        } else {
          throw new NotFoundException(
            `Member account '${memberAccount}' not found in iCafeCloud.`,
          );
        }
      } catch (error) {
        if (error instanceof NotFoundException) {
        ...
        this.logger.error(
          `Failed to verify member account ${memberAccount}: ${error.message}`,
        );
        throw new InternalServerErrorException(
          `Failed to verify member account before finding notes.`,
        );
      }
    }
