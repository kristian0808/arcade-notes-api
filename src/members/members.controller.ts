...
import {
  MemberRankingsQueryDto,
  TimeframeEnum,
} from './dto/member-rankings-query.dto';
...
export class MembersController {
  ...
  async searchMembers(
    ...
    this.logger.log(
      `Received request for GET /api/members/search?query=${accountName}`,
    ); // Log message might need update later
    if (!accountName?.trim()) {
    ...
    try {
      ...
      if (error instanceof NotFoundException) {
        this.logger.warn(
          `Member search for "${accountName}" returned no results (404).`,
        );
        return [];
      }
      this.logger.error(
        `Failed to search members for query "${accountName}": ${error.message}`,
        error.stack,
      );
  ...
  async getMemberRankings(
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch member rankings: ${error.message}`,
        error.stack,
      );
  ...
  async getMemberById(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get member by ID ${memberId}: ${error.message}`,
        error.stack,
      );
      if (error instanceof NotFoundException) {
        throw error;
